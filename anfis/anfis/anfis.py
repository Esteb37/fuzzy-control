...
class ANFIS:
    ...
    def trainHybridJangOffLine(self, epochs=5, tolerance=1e-5, initialGamma=1000, k=0.01, print_mfs=False):
        ...
        while (epoch < epochs) and (convergence is not True):
            ...
            print(str(epoch) + ') MSEe: ' + str(error))
            ...
            memFuncsCopy = copy.deepcopy(self.memFuncs)
            mem_funcs.append(memFuncsCopy)
            epoch = epoch + 1
        ...
        # min error
        print('Min error: ' + str(np.min(self.errors)) +
              ' at epoch ' + str(np.argmin(self.errors) + 1))

        if print_mfs:
            print("Best membership functions: ")
            print(mem_funcs[np.argmin(self.errors)])
